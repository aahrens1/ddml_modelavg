...
program define mysim , rclass
	...
	*** ddml partial
	...
	ddml E[Y|X]: pystacked $Y $X 								|| ///
					method(ols) xvars($xvar_poly2)				|| ///
					method(ols) xvars($xvar_spline)				|| ///
					m(lassocv) xvars($xvar_poly2) 				|| ///
					m(ridgecv) xvars($xvar_poly2) 				|| ///
					m(lassocv) xvars($xvar_spline) 				|| ///
					m(ridgecv) xvars($xvar_spline) 				|| ///
					m(rf) pipe(sparse) opt($rflow) 				|| ///
					m(rf) pipe(sparse) opt($rfhigh) 			|| ///
					m(gradboost) pipe(sparse) opt($gradlow) 	|| ///
					m(gradboost) pipe(sparse) opt($gradhigh)    || ///
					m(nnet) opt($nnetopt) pipe(stdscaler), $pyopt 
